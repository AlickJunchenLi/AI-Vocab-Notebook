...
void wordsDictionary::createAllChineseSynonyms() {
    ...
    while (getline(fin, line)) {
        ...
        string token;
        while (zhAiThesaurusWord::getTokens(iss, token)) {
            trim(token);
            if (database.find(token) == database.end()) {
                aiThesaurusWord *tmp_word_store = new zhAiThesaurusWord(token);
...
void wordsDictionary::createAllEnglishSynonyms() {
    ...
    while (getline(fin, line)) {
        ...
        vector <string> tokens;
        string token;
        while (enAiThesaurusWord::getTokens(iss, token)) {
            trim(token);
            if (database.find(token) == database.end()) {
                aiThesaurusWord *tmp_word_store = new enAiThesaurusWord(token);
                database.emplace(token, tmp_word_store);
...
void wordsDictionary::initializeChineseSynonyms() {
    ...
    while (getline(fin, line)) {
        ...
        for (size_t idxHeadword = 0; idxHeadword < tokens.size(); ++idxHeadword) {
            ...
            aiThesaurusWord *headNote = database[headword];
            for (size_t idxSynonym = 0; idxSynonym < tokens.size(); ++idxSynonym) {
                if (idxSynonym == idxHeadword) {
                    continue;
                }
                const string &syn = tokens[idxSynonym];
                aiThesaurusWord *synNode = database[syn];
                headNote->aiWordSynonyms.insert(synNode);
            }
        }
...
void wordsDictionary::initializeTranslations() {
    auto fin_ptr = loadTranslationFile();
    ifstream &fin = fin_ptr;
    if (!fin.is_open()) { cerr << "..."; return; }

    auto stripTag = [](string &s) {
        if (!s.empty() && s.front() == '[') {
            auto pos = s.find(']');
            if (pos != string::npos) s.erase(0, pos + 1)
        }
    }
}
